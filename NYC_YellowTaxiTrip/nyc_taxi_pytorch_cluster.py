...
def process_files(rank, world_size, file_paths, hdfs_host, hdfs_port, read_block_size, data_loader_batch_size, n_clusters, output_file):
    ...
    dist.all_gather([result["cluster_data"] for result in local_results], gathered_cluster_data)
    dist.all_gather([result["metrics"] for result in local_results], gathered_metrics)
