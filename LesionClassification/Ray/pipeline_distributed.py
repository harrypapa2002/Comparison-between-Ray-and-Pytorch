...
# Function to read tabular data from HDFS
def load_tabular_data_hdfs(hdfs_host, hdfs_port, file_path):
    hdfs = fs.HadoopFileSystem(host=hdfs_host, port=hdfs_port)
    with hdfs.open_input_file(file_path) as file:
        return pd.read_excel(file)

# Function to save intermediate results back to HDFS (e.g. feature vectors)
def save_to_hdfs(data, hdfs_host, hdfs_port, hdfs_path):
    hdfs = fs.HadoopFileSystem(host=hdfs_host, port=hdfs_port)
    with hdfs.open_output_stream(hdfs_path) as file:
        np.save(file, data)
...
def feature_vector_extraction_hdfs(dataset, hdfs_host, hdfs_port, image_folder):
    # Initialize HDFS
    hdfs = fs.HadoopFileSystem(host=hdfs_host, port=hdfs_port)
    ...
    def preprocess_image_from_hdfs(image_path):
        """Preprocess an image for ResNet50 from HDFS."""
    ...
    for _, row in tqdm(dataset.iterrows(), total=len(dataset), desc="Processing images"):
        ...
        try:
            # Preprocess the image
            preprocessed_img = preprocess_image_from_hdfs(image_path)
            # Ensure the tensor is on the same device as the model
            with torch.no_grad():  # Disable gradient computation
                features = feature_extractor(preprocessed_img)  # Extract features
            feature_vectors.append(features.squeeze().numpy())  # Flatten the output and convert to NumPy
...
def main():
    ...
    distributed_pipeline(tabular_data_path, image_folder)
